...
AND auth_key_id NOT IN (
  ...
  name
			...
			},
			{
				...
				// untagged nodes can carry tags='null'. Exclude that spelling,
				// otherwise untagged nodes lose their user during migration.
...
WHERE tags IS NOT NULL AND tags != '[]' AND tags != '' AND tags != 'null';
			...
			{
				// Clear zero-time node expiry values to NULL. Older versions
				// could persist a pointer to time.Time{} instead of NULL, and
				// newer clients/server paths treat that as already expired.
...
WHERE expiry IS NOT NULL AND expiry < '1900-01-01';
			...
			},
			{
				...
				// version of 202602201200-clear-tagged-node-user-id. Recovery is
				// best-effort: nodes registered with a pre-auth key can be
				// restored from that key's user_id.
				// Fixes: https://github.com/juanfont/headscale/issues/3323
